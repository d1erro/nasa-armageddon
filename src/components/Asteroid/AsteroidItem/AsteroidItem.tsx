...
const AsteroidItem: FC<IAsteroidItemProps> = ({
    ...
    const missDistanceLunar = stringToRoundedNum(
        data.close_approach_data[0].miss_distance.lunar,
    )

    const missDistanceKm = stringToRoundedNum(
    ...
    return (
        <div className={styles.asteroidItem}>
            <h4 className={styles.date}>
                {formatDate(data.close_approach_data[0].close_approach_date)}
            </h4>
                ...
                <div className={styles.distance}>
                    <div>
                        {distanceUnit === 'km' ? (
                            ...
                            <p>
                                {missDistanceLunar}{' '}
                                {formatDistanceText(missDistanceLunar)}
                            </p>
                        )}
            ...
            <div className={styles.buttonAndDangerInfo}>
                {!isInCard ? (
                    ...
                    <button className={styles.inCartButton}>В КОРЗИНЕ</button>
                )}
